...
NEXT_PUBLIC_LMSTUDIO_API_KEY=현재 프로젝트 .env.local의 값을 복사
...
- 브라우저/프록시 CORS 통과를 위해 `Origin`과 `Referer`를 `https://lm.alluser.kr` 기준으로 함께 보냅니다.
- API 키는 저장소 코드에 하드코딩하지 말고 로컬의 `.env.local` 또는 Netlify 환경변수에서 관리합니다. `NEXT_PUBLIC_` 값은 브라우저에 노출되므로 완전한 비밀값으로 간주하지 않습니다.
...
export function getLocalLLMRequestHeaders(modelConfig) {
    const origin = modelConfig.apiUrl || "https://lm.alluser.kr";
    const headers = {
...
```bash
curl https://lm.alluser.kr/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "Origin: https://lm.alluser.kr" \
  -H "Referer: https://lm.alluser.kr/" \
  -H "X-API-Key: <NEXT_PUBLIC_LMSTUDIO_API_KEY>" \
  -d '{"model":"gemma-4-12b-it","messages":[{"role":"user","content":"hi"}],"reasoning_effort":"none","stream":false}'
```

## 다른 프로젝트에 옮길 때 체크리스트

1. `.env.local` 또는 Netlify에 `NEXT_PUBLIC_LMSTUDIO_API_URL`, `NEXT_PUBLIC_LMSTUDIO_API_KEY`를 추가합니다.
2. 모델 선택 UI에는 위 4개 로컬 모델을 노출하고, 앱 기본값은 `upstage:solar-pro4`, 로컬 대체 모델은 `lmstudio:gemma-4-12b-it`로 둡니다.
...
4. 요청 URL은 `${NEXT_PUBLIC_LMSTUDIO_API_URL}/v1/chat/completions`로 조립합니다.
