...
- [x] **공통 유틸리티 파일 생성 (`utils/textProcessor.js`)**
    - [x] 동적 버퍼 비율 계산 함수 (`getBufferRatio`)
        - 100자 이하: 70%
        - 150자 이하: 75%
        - 200자 이하: 80%
        - 300자 이하: 85%
        - 300자 초과: 90%
    - [x] AI 프롬프트용 글자수 계산 함수 (`getPromptCharLimit`)
    - [x] 글자수별 맞춤 프롬프트 지침 생성 함수 (`getCharacterGuideline`)
        - 100자 이하: 2~3개 짧은 문장, 15자 이내, 완전한 종결어미 강조
        - 200자 이하: 4~5개 짧은 문장, 20자 이내
        - 200자 초과: 일반적인 글자수 제한 지침
    - [x] AI 출력 메타 정보 제거 함수 (`cleanMetaInfo`)
    - [x] 완전한 문장 단위 트리밍 함수 (`truncateToCompleteSentence`)
        - 한국어 종결어미 패턴 인식 (~함, ~음, ~임, ~됨 등)
        - 문장 완결성 보장 (마침표로 끝나는 완전한 문장까지만 출력)
        - 500자 절대 상한선 적용

- [x] **4개 페이지에 공통 유틸리티 적용**
    - [x] 과세특 (`/gwasetuk`) ✅
    - [x] 동아리 세특 (`/club`) ✅
    - [x] 행발 작성 (`/behavior`) ✅
    - [x] 가정통신문 (`/letter`) ✅
...
- [x] **적용 내용**
    - [x] import 추가: `textProcessor.js`에서 함수 가져오기
    - [x] 기존 로컬 `cleanMetaInfo`, `truncateToCharLimit` 함수 제거
    - [x] `generatePrompt` 함수: `lengthInstruction` 변수를 `getCharacterGuideline(targetChars)`로 변경
    - [x] `generateForStudent` 함수: 후처리 로직을 `truncateToCompleteSentence` 사용으로 변경

### 적용 효과
| 기능 | 설명 |
|------|------|
| 동적 버퍼 | 100자 이하 70%, 200자 이하 80%, 300자 이하 85%, 그 외 90% |
| 극소 글자수 지침 | 100자 이하: 2-3개 짧은 문장, 15자 이내, ~함. ~음. ~임. 종결 |
| 문장 완성 보장 | 한국어 종결어미 패턴 인식 후 완전한 문장 단위로 트리밍 |
| 500자 상한선 | 절대로 500자(1500 bytes) 초과 불가 |
